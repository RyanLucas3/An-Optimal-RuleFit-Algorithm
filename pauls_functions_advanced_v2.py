...
def generate_tree(X_train, y_train, X_test , y_test , n_num, feat_size,
                  max_iter_hy, depth_bi=False, depth_hy=False, depth_grid=False, complexity_bi=False,
    ...
    for i in range(1, n_num + 1):
        ...
        if Clas_CART == True:
            ...
            if complexity_bi > 0 and depth_bi > 0:
                ...
                model.fit(X_train_sample, y_train)
            model = iai.OptimalTreeClassifier(**model.get_best_params(), max_depth=depth_bi)
            model.fit(X_train_sample, y_train)
        ...
        if OCT == True:
            for depth in depth_grid:
                if complexity_bi == False and depth_bi == False:
                    ...
                    model.fit(X_train_sample, y_train)
                if complexity_bi == False and depth_bi > 0 :
                    model = iai.GridSearch(iai.OptimalTreeClassifier(max_depth = depth))
                    model.fit(X_train_sample, y_train)
                if complexity_bi > 0  and depth_bi == False:
                ...
                if complexity_bi > 0 and depth_bi > 0:
                    ...
                    model.fit(X_train_sample, y_train)
                model = iai.OptimalTreeClassifier(**model.get_best_params(),max_depth=depth)
                model.fit(X_train_sample, y_train)
